...
struct DateTimeFormatTests {
    ...
    func dateTimeOffsetFormatting(offsetSeconds: Int, expected: String) throws {
    ...
    func dateTimeDefaultFormatting() throws {
    ...
    func dateTimeFractionAndOffset() throws {
    ...
    func naiveDateTimeDescription() throws {
        let date = try #require(NaiveDate(year: 2025, month: 12, day: 25))
        let time = try #require(NaiveTime(hour: 14, minute: 30, second: 05, nanosecond: 123_456_789))
    ...
    func paddingCheck() throws {
        let dt = try NaiveDateTime(
            date: #require(.init(year: 8, month: 3, day: 9)),
            time: #require(.init(hour: 4, minute: 5, second: 6, nanosecond: 7))
        )
